...
GM_addStyle(`
	...
	#hatbar_time_fix, .hatbar_form { background: #ffffff; border: 1px solid #aaaaaa; width: 150px; font-weight: normal; }
`);
...
function vip() {
	if (get('vip')) {
		$('#hatbar').append('<div style="position: absolute; top: 5px; left: 20px;"><input type="time" min="06:00" max="10:00" id="hatbar_start" class="hatbar_form" /></div>');
		$('#hatbar_start').val(get('start'));
		$('#hatbar_start').change(function () {
			set('start', $(this).val());
		});
		if (get('start')) {
			refresh = setInterval(function () { location.reload(); }, 300000); // 5 minutes
			start_stop = setInterval(function () { vipCheck(); }, 20000); // 20 seconds
		} else if (refresh) {
			stopInterval(refresh);
			stopInterval(start_stop);
		}
	}
}

function vipCheck() {
	var start = parseTime(get('start') + ':00');
	var stop = start + (8 * 3600 * 1000);
	var now = parseTime(new Date().toLocaleTimeString('en-US', { hour12: false, hour: "numeric", minute: "numeric", second: "numeric" }));
	if (start <= now && now >= (6 * 3600 * 1000) && now <= (10 * 3600 * 1000) && $('.sublnk.w span small').html() != 'I am working') {
		document.location.href = '/activities/set-work/8';
	}
	if (stop <= now && $('.sublnk.w span small').html() == 'I am working') {
		document.location.href = '/users/logout-finish';
	}
}
